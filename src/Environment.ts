...
export function showLocalEnv (env : Core.Term) : string {
    ...
    if (!Core.isPair(env.fst)) throw new Error(`Expected pair in Env not ${env.fst.kind}`);
    let curr = env.fst;
    if (Core.isNative(curr.snd) || Core.isFExpr(curr.snd)) return '~';
    return `${Parser.deparse(curr.fst)}:${curr.snd.kind.substr(0, 4).toLowerCase()} ${showEnv( env.snd )}`
}

export function lookup (t : Core.Term, env : Core.Term) : Core.Term {
    ...
    console.log(` /lookup/ ? ${Parser.deparse(t)} IN ${showEnv(env)}`);
    let curr = env.fst;
    console.log(` /lookup/ ? ${Parser.deparse(t)} EQ ${Parser.deparse(curr.fst)}`);
    if (Core.equalTo(t, curr.fst)) {
        console.log(` !lookup! @ ${Parser.deparse(t)} => ${Parser.deparse(curr.snd)}`);
        return curr.snd;
    }
    return lookup( t, env.snd );
}
