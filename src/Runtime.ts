...
export function evaluate (t : Core.Term, env : Core.Term, depth : number = 0) : Core.Term {
    ...
    case Core.isPair(t)    :
        let fst = evaluate( t.fst, env, depth + 1 );
        let snd = t.snd;
        console.log(`${indent} $ eval – ... is Pair! ${Parser.deparse(fst)}`);
        let rest = evaluate( snd, env, depth + 1 );
        console.log(`◯ [${depth.toString().padStart(3, '0')}] `+'–'.repeat(72));
        console.log(`${indent} $ eval – Pair( fst: ${Parser.deparse(fst)}, snd: <${rest.kind}> ${Parser.deparse(rest)} )`);
        return Core.Pair( fst, rest );
    case Core.isCons(t)    :
        console.log(`${indent} $ eval – Apply or Cons? (${t.head.kind}, ${t.tail.kind})`);
        let head = evaluate( t.head, env, depth + 1 );
        let tail = t.tail;
        ...
        default:
            console.log(`${indent} $ eval – ... is Cons! ${Parser.deparse(head)}`);
            if (Core.isNil(tail)) return Core.Cons( head );

            let rest = evaluate( tail, env, depth + 1 );
            console.log(`◯ [${depth.toString().padStart(3, '0')}] `+'–'.repeat(72));
            console.log(`${indent} $ eval – Pair( fst: ${Parser.deparse(head)}, snd: <${rest.kind}> ${Parser.deparse(rest)} )`);
            if (!Core.isCons(rest)) throw new Error(`WTF! Eval Cons should return Cons not ${rest.kind}`);
            return Core.Cons( head, rest );
        }
    default:
...
export function initEnv () : Core.Term {
    ...
    const liftComparisonBinOp = (op : string, f : (n : number | string, m : number | string) => boolean) : Core.NativeFunc => {
        return (arg : Core.Term) : Core.Term => {
            ...
            if (!Core.isList(arg)) throw new Error(`Expected Pair in &:(${op}) not(${arg.kind})`);
            if (!Core.isList(arg.tail)) throw new Error(`Expected rhs to be Pair in &:(${op}) not(${arg.tail.kind})`);
            let lhs = arg.head;
            let rhs = arg.tail.head;
    ...
    env = Environment.define(
        Core.Sym('=='), Core.Native((arg : Core.Term) : Core.Term => {
            ...
            if (!Core.isList(arg)) throw new Error(`Expected Pair in &:(==) not(${arg.kind})`);
            if (!Core.isList(arg.tail)) throw new Error(`Expected rhs to be Pair in &:(==) not(${arg.tail.kind})`);
            let lhs = arg.head;
            let rhs = arg.tail.head;
            return Core.equalTo( lhs, rhs ) ? Core.True() : Core.False();
        }),
        env
    );
    ...
    env = Environment.define(
        Core.Sym('!='), Core.Native((arg : Core.Term) : Core.Term => {
            ...
            let lhs = arg.head;
            let rhs = arg.tail.head;
            return Core.equalTo( lhs, rhs ) ? Core.False() : Core.True();
        }),
        env
    );
    ...
    env = Environment.define(
        Core.Sym('let'), Core.FExpr((arg : Core.Term, env : Core.Term) : [ Core.Term, Core.Term ] => {
            ...
            return [
                body.head,
                Environment.define( sym, evaluate( value, env, -1 ), env )
            ];
        }),
        env
    );
